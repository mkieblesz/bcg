...
class ImageEndpointsTestCase(TestCase):
    ...
    def test_convert_to_extension(self):
        ...
        url = reverse('get-converted-image', kwargs={'image_id': image.id, 'ext': 'png'})
        response = self.client.get(url)
        assert Image.objects.count() == 2
        converted_image = Image.objects.last()
        assert response.json() == {'id': image.id, 'url': converted_image.get_image_url()}
        assert converted_image.image_file.name.split('.')[0] == image.image_file.name.split('.')[0]